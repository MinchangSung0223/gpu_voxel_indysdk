...
def main():
	...
	p.resetBasePositionAndOrientation(d435Id, [0.34 ,0.64, 1.2],p.getQuaternionFromEuler([-pi/32,pi/6+pi/16,-pi/4-pi/12]))
	...
	t.start()
	T = getHomogeneousMatrix(d435Id)
	print("[["+str(T[0,0])+","+str(T[0,1])+","+str(T[0,2])+","+str(T[0,3])+"]"
	+",\n["+str(T[1,0])+","+str(T[1,1])+","+str(T[1,2])+","+str(T[1,3])+"],"
	+"["+str(T[2,0])+","+str(T[2,1])+","+str(T[2,2])+","+str(T[2,3])+"],"
	+"["+str(T[3,0])+","+str(T[3,1])+","+str(T[3,2])+","+str(T[3,3])+"]]");
	rate=rospy.Rate(10);
	...
	while(1):
		...
		for j in range(1,6):
			p.resetJointState(indy7robotId, j, targetPosition[j-1])
